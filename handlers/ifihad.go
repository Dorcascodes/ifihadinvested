...
func Ifihad(w http.ResponseWriter, r *http.Request) {
	...
	case "/":
		trending(w, r)
	case "/hodling":
		http.Redirect(w, r, "/hodling/", http.StatusMovedPermanently)
	case "/hodling/":
		tpl.ExecuteTemplate(w, "hodling.html", nil)
	case "/compare":
		http.Redirect(w, r, "/compare/", http.StatusMovedPermanently)
	case "/compare/":
		tpl.ExecuteTemplate(w, "compare_coins.html", nil)
	default:
